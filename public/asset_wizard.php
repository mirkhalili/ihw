...
$stageFields=['printer'=>['PrinterCount','DefaultPrinterName','PrinterNames','PrinterPorts','PrinterDrivers','DuplexPrinters','PrinterDetails'],'scanner'=>['ScannerCount','ScannerNames','ScannerManufacturers','ScannerDetails']];
...
function splitHardwareRecords(array $row,string $type,CsvImporter $importer):array{
 ...
 if(!$parts&&$details!=='')$parts=[$details];$out=[];foreach($parts as $i=>$part){$r=$row;$r['MonitorDetails']=$part;$r['_unit_index']=$i+1;$out[]=$r;}return $out;
}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if($step!=='review'&&$action==='analyze'){
  try{
   ...
   $rows=[];foreach($importer->rows($_FILES['csv']['tmp_name']) as $r){$rows[]=$r;if($step==='computer')break;}
   if(!$rows)throw new RuntimeException('CSV داده‌ای ندارد.');
   $row=$rows[0];$records=splitHardwareRecords($row,$step,$importer);
   foreach($records as &$r){$r['_source_file']=basename($_FILES['csv']['name']);unset($r['_skipped']);if(in_array($step,['printer','scanner'],true)){ $r['IPAddresses']=[];unset($r['MACAddresses']);}}
   unset($r);$wizard['records'][$step]=$records;
   if($step==='computer'&&!empty($row['MonitorDetails']))$wizard['records']['display']=splitHardwareRecords($row,'display',$importer);
   $_SESSION['_hardware_wizard']=$wizard;redirect('asset_wizard.php?step='.$step);
  }catch(Throwable $e){$error=$e->getMessage();}
 }
 if($step!=='review'&&$action==='skip'){
  $wizard['records'][$step]=['_skipped'=>true];$_SESSION['_hardware_wizard']=$wizard;$i=array_search($step,$steps,true);redirect($i<count($steps)-1?'asset_wizard.php?step='.$steps[$i+1]:'asset_wizard.php?step=review');
 }
