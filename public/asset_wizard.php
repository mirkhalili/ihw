...
if ($step !== 'review' && !can_manage_asset_type($step, 'create')) { http_response_code(403); exit('برای این پایگاه دسترسی ثبت ندارید.'); }
...
$labels = [
    ...
    'PrinterType'=>'نوع چاپگر','PrinterCapabilities'=>'قابلیت‌ها','ColorMode'=>'رنگی/سیاه‌وسفید','Connection'=>'نوع اتصال','NetworkSharing'=>'اشتراک‌گذاری شبکه','Duplex'=>'چاپ دورو','DisplayTechnology'=>'فناوری نمایشگر','DisplaySize'=>'اندازه نمایشگر','DisplayResolution'=>'رزولوشن','DisplaySerial'=>'سریال نمایشگر',
    'ScannerType'=>'نوع اسکنر','ScannerResolution'=>'رزولوشن اسکنر','ScannerConnection'=>'اتصال اسکنر','ScannerDuplex'=>'اسکن دورو'
];
    ...
    if ($step === 'review' && $action === 'finish') {
        try {
            foreach ($steps as $type) {
                if (empty($wizard['records'][$type]['asset_no'])) throw new RuntimeException('مرحله '.$types[$type].' تکمیل نشده است.');
            }
            $seen = [];
            foreach ($steps as $type) {
            ...
            $db->commit();
            unset($_SESSION['_hardware_wizard']);
            flash('success','هر چهار پایگاه با موفقیت ثبت شدند.');
            redirect('index.php');
        } catch (Throwable $e) {
